 ...
 */
public class MainSchemaAsGraph extends JFrame implements WindowListener, ActionListener {
	 ...
	 * Construct the gui.
	 */
	public MainSchemaAsGraph() {
		super("Schema to graph converter");
	}
	...
	public static void main(String[] args) {
		MainSchemaAsGraph main = new MainSchemaAsGraph();
		main.init();
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(source.equals(jButtonAnalyze)){
			if(schemaFileSelected == null)
				...
				try {
					xmlSchemaRelations = new XMLSchema(schemaFileSelected.getAbsolutePath());
	
					textAreaQuery.setText("");
					
					for(GraphNode node : xmlSchemaRelations.getGraphNodes()){
						textAreaQuery.setText(textAreaQuery.getText() + node + "\n");
					...
					try {
						Object root = xmlSchemaRelations.getGraph().buildGraphRepresentation(graph);
						if(root != null)
							((mxCell)root).setStyle("line;strokeColor=#FF0000");
						mxGraphLayout graphLayout = new mxHierarchicalLayout(graph);
						graphLayout.execute(graph.getDefaultParent());
						
						graphComponent = new mxGraphComponent(graph);
						
						if(splitPaneForTextAreas.getComponentCount() > 2)
							splitPaneForTextAreas.remove(2);
						
						splitPaneForTextAreas.add(graphComponent);
						splitPaneForTextAreas.validate();
						splitPaneForTextAreas.revalidate();
					}
